    ...
    if (!error && response.statusCode == 200){
        ...
        $('.col.search_capsule').each(function(i,element) { 
            ...
            request('https://steamcommunity.com/market/search?q=&category_753_Game%5B%5D=tag_app_' + gameId + '&category_753_cardborder%5B%5D=tag_cardborder_0&category_753_item_class%5B%5D=tag_item_class_2&appid=753', (error, response, html)=> {
                if (!error && response.statusCode == 200){
                    const $ = cheerio.load(html);
                    let tradingCardPrice = $('.normal_price').text();
                    let gameName = $('.market_searchedForTerm').text();

                    if (tradingCardPrice){
                        console.log("precio del cromo para " + gameName + " :" + tradingCardPrice);
                    } else {
                        console.log( gameName + " does not have trading cards");
                    }
                }
            })

            gameList.push(gameId); // with this line appends EVERY game
        });
